...
function Orders(props) {
  ...
  const subtotal = useInputFormData('');
  const total = useInputFormData('');
    ...
    <div>
      <div className="flex flex-row font-sans container mx-auto h-full  justify-center items-center mt-20">
        ...
        <div className="bg-blue-500 text-white shadow-md rounded px-20 pt-10 pb-12 mb-4">
          ...
          <input className="bg-white text-blue-500 font-bold py-2 px-4 mt-10 rounded" type="button" value={loading ? 'Loading...' : 'Submit Order'} onClick={handleOrders} disabled={loading} /><br />
        </div>
      </div>
    </div>
  );
...
const useInputFormData = initialValue => {
